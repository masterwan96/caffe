...
void NormalizeLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
    ...
    if (this->blobs_.size() > 0) {
        ...
        std::cout << "the scale: " << this->blobs_[0]->cpu_data()[0] << std::endl;
    }
    if (channel_shared_) {
...
void NormalizeLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
    ...
    buffer_.Reshape(1, bottom[0]->channels(), bottom[0]->height(), bottom[0]->width());
    if (!across_spatial_) { norm_.Reshape(bottom[0]->num(), 1, bottom[0]->height(), bottom[0]->width()); }
...
void NormalizeLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
    ...
    for (int n = 0; n < num; ++n) {
       ...
       * N: hw
       * alpha = 1
       * A = buffer_data(x^2) (c*hw)
       * x = sum_channel_multiplier(channle*1, value=1)
       * beta = 1
       * norm_data 初始值为eps
       * lda = M = channel --> A 转置
       * norm_data=1*buffer_data*sum_channel_multiplier+1*norm_data
       * nom_data: (hw*1)
       * 这里的norm_data相当于在channel维度上对元素进行了求平方和，
       * 然后结果存在norm_data中（存了spatial_dim也就是height*width个）
       * */
            caffe_cpu_gemv<Dtype>(CblasTrans, channels, spatial_dim, Dtype(1), buffer_data, sum_channel_multiplier,
                                  Dtype(1), norm_data);
            /**
       ...
       * C=alpha*A*B+beta*C
       * M: channel
       * N: spatial_dim(h*w hw)
       * K: 1
       * alpha: 1
       * A: sum_channel_sumltiplier( 1*chanell  value=1)
       * B: norm_data ( hw * 1)
       * beta: 0
       * C: buffer_data
       * lda = K
       ...
       * buffer_data (c * hw)
       * */
            caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, channels, spatial_dim, 1, Dtype(1),
                                  sum_channel_multiplier, norm_data, Dtype(0), buffer_data);
            //
       ...
       * */
            caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, channels, spatial_dim, 1, Dtype(1), scale,
    ...
    if (this->param_propagate_down_[0]) {
        ...
        if (channel_shared_) {
            ...
            for (int n = 0; n < num; ++n) {
                ...
                caffe_cpu_gemv<Dtype>(CblasNoTrans, channels, spatial_dim, Dtype(1), buffer_data,
                                      sum_spatial_multiplier, Dtype(0), buffer_channel);
    ...
    if (propagate_down[0]) {
        for (int n = 0; n < num; ++n) {
            if (across_spatial_) {
                ...
                // scale bottom_diff
                caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, channels, spatial_dim, 1, Dtype(1),
                                      sum_channel_multiplier, buffer_spatial, Dtype(0), buffer_data);
