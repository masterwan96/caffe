    ...
    if ((!scale_param && propagate_down[1]) || (scale_param && this->param_propagate_down_[0])) {
        ...
        if (!is_eltwise) {
            ...
            if (inner_dim_ == 1) {
                ...
                caffe_cpu_gemv(CblasNoTrans, sum_result_.count(), inner_dim_, Dtype(1), product, sum_mult, Dtype(0),
                               sum_result);
            }
            if (outer_dim_ != 1) {
                const Dtype *sum_mult = sum_multiplier_.cpu_data();
                ...
                if (scale_dim_ == 1) {
                    ...
                    caffe_cpu_gemv(CblasTrans, outer_dim_, scale_dim_, Dtype(1), sum_result, sum_mult,
